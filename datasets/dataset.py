...
SUPPORTED_DATASETS = {
    ...
    'unc_F': {
        'splits': ('train', 'val', 'test')},
    'referit_F': {'splits': ('train', 'val', 'test')},
    'hy': {'splits': ('train', 'val', 'test')},
    'hy_rsvg': {'splits': ('train', 'val', 'test')},
    'hyhb_rsvg': {'splits': ('train', 'val', 'test')},
    'rsvg': {'splits': ('train', 'val', 'test')},
    'rsvg22': {'splits': ('train', 'val', 'test')},
    'LQVG': {'splits': ('train', 'val', 'test')},
    'myov_rsvg': {'splits': ('train', 'val', 'test')},
    'rs_ov_try': {'splits': ('train', 'val', 'test')},
}
...
class VGDataset(Dataset):
    def __init__(self, data_root, split_root='data', dataset='referit', transforms=[],
        ...
        # setting datasource
        if self.dataset == 'referit' or self.dataset == 'referit_F':
            self.dataset_root = osp.join(self.data_root, 'referit')
            self.im_dir = osp.join(self.dataset_root, 'images')
        elif self.dataset == 'flickr' or self.dataset == 'flickr_F':
            self.dataset_root = osp.join(self.data_root, 'Flickr30k')
            self.im_dir = osp.join(self.dataset_root, 'flickr30k-images')
        elif self.dataset == 'hy':
            self.dataset_root = osp.join(self.data_root, 'hy')
            self.im_dir = osp.join(self.dataset_root, 'hy')
        elif self.dataset == 'hy_rsvg':
            self.dataset_root = osp.join(self.data_root, 'hy_rsvg')
            self.im_dir = osp.join(self.dataset_root, 'hy_rsvg')
        elif self.dataset == 'hyhb_rsvg':
            self.dataset_root = osp.join(self.data_root, 'hyhb_rsvg')
            self.im_dir = osp.join(self.dataset_root, 'hyhb_rsvg')
        elif self.dataset == 'rs_ov_try':
            self.dataset_root = osp.join(self.data_root, 'rs_ov_try')
            self.im_dir = osp.join(self.dataset_root, 'rs_ov_try')
        elif self.dataset == 'rsvg' or self.dataset == 'myov_rsvg':
            self.dataset_root = osp.join(self.data_root, 'rsvg')
            self.im_dir = osp.join(self.dataset_root, 'JPEGImages')
        elif self.dataset == 'rsvg22' or self.dataset == 'LQVG':
            self.dataset_root = osp.join(self.data_root, 'rsvg22')
            self.im_dir = osp.join(self.dataset_root, 'images')
        else:  # refer coco etc.
        ...
        if self.dataset == 'flickr' or self.dataset == 'flickr_F':
        ...
        self.covert_bbox = []
        if not (self.dataset == 'referit' or self.dataset == 'referit_F' or self.dataset == 'flickr' or self.dataset == 'flickr_F'):  # for refcoco, etc
            # xywh to xyxy
