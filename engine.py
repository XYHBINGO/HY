...
def evaluate(model, criterion, postprocessor, data_loader, device, save_path=''):
    ...
    for iteration, (img, mask, target) in enumerate(tqdm(prefetcher)):
        ...
        img_n = data_loader.dataset.imgset_info[target_dict['image_idx']][0]
        text_n = data_loader.dataset.imgset_info[target_dict['image_idx']][3]
        im = cv2.imdecode(np.fromfile('/home/xyhbingo/XYH_workplace/HY/WCL/hy/data/rs_ov_try/rs_ov_try/%s'%img_n, dtype=np.uint8), -1)
        cv2.rectangle(im, (int(pred_boxes[0][0]), int(pred_boxes[0][1])), (int(pred_boxes[0][2]), int(pred_boxes[0][3])), (255, 0, 0), 5)
        cv2.imwrite(os.path.join('/home/xyhbingo/XYH_workplace/HY/WCL/hy/hy_ov/%s_%s.jpg' % (img_n.split('.')[0], text_n)), im)

        ious = box_ops.box_pair_iou(gt_bbox, pred_boxes)[0]
        #if ious < 0.5 and len(all_data_5) <= 103:
        #    all_data_5.append(data_loader.dataset.imgset_info[target_dict['image_idx']])
        sum_iou = ious.sum()
    ...
    #torch.save(all_data_5, 'hy_35.pth')
    if save_path:
